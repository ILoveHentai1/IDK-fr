...
function App(){
 ...
 const carAsset=(model)=>{
   const key=String(model||'MK6').toUpperCase().replace('.','').replace(' ','-');
   const files={'MK5':'gti-mk5.svg','MK6':'gti-mk6.svg','MK7':'gti-mk7.svg','MK75':'gti-mk75.svg','MK8':'gti-mk8.svg','GOLF-R':'golf-r.svg','GTI':'gti-mk6.svg'};
   return `./${files[key]||'gti-mk6.svg'}`;
 };
 const icon=(n,m,h,other=false,model=carModel,color=carColor)=>L.divIcon({className:'',iconSize:[76,66],iconAnchor:[38,50],html:`<div class="car ${other?'other':''} color-${color}"><div class="tag">${n} · ${Math.round(m)} MPH</div><div class="modelTag">${model}</div><img src="${carAsset(model)}" style="transform:rotate(${h||0}deg)"></div>`});
 ...
 return <div className={`app ${appMode==='fish'?'fishMode':'driveMode'}`}>
   <div ref={mapEl} className="map"/>

   <aside className="desktopRail">
     <div className="railLogo"><span>GTI</span><b>NAV</b><small>by Nova</small></div>
     <button className={appMode==='drive'&&activeTab==='drive'?'active':''} onClick={()=>{setAppMode('drive');setActiveTab('drive')}}>⌂<span>Home</span></button>
     <button onClick={()=>map.current?.flyTo(pos,17)}>⌖<span>Map</span></button>
     <button className={appMode==='drive'&&activeTab==='convoy'?'active':''} onClick={()=>{setAppMode('drive');setActiveTab('convoy')}}>◉<span>Convoy</span></button>
     <button className={appMode==='drive'&&activeTab==='garage'?'active':''} onClick={()=>{setAppMode('drive');setActiveTab('garage')}}>▣<span>Garage</span></button>
     <button className={appMode==='drive'&&activeTab==='history'?'active':''} onClick={()=>{setAppMode('drive');setActiveTab('history')}}>◷<span>Drives</span></button>
     <button className={appMode==='fish'?'active fishRail':''} onClick={()=>{setAppMode('fish');localStorage.setItem('gtiAppMode','fish')}}>🎣<span>Nova Fish</span></button>
   </aside>

   <header className="top">
     <div className="modeBrand">
       <button className={appMode==='drive'?'active':''} onClick={()=>{setAppMode('drive');localStorage.setItem('gtiAppMode','drive')}}>🚗 GTI NAV</button>
       <button className={appMode==='fish'?'active fish':''} onClick={()=>{setAppMode('fish');localStorage.setItem('gtiAppMode','fish')}}>🎣 NOVA FISH</button>
     </div>
     <div className="searchWrap">
       <form onSubmit={go}>
         <span className="searchIcon">⌕</span>
         <input value={dest} onChange={e=>setDest(e.target.value)} placeholder={appMode==='fish'?'Search coast, lake, town…':'Search destination…'}/>
         <button>{searching?'…':'GO'}</button>
       </form>
       {suggestions.length>0&&<div className="suggestions">
         {suggestions.map((s,i)=><button key={s.place_id||i} onClick={()=>routeToPlace(s)}>
           <span>⌖</span><span><strong>{s.name||s.display_name.split(',')[0]}</strong><small>{s.display_name}</small></span>
         </button>)}
       </div>}
     </div>
     <div className="topTelemetry"><span>{Math.round(speed)}<small>MPH</small></span><span>{convoyLive?'● LIVE':'○ SOLO'}</span></div>
   </header>

   <div className="status">{status}</div>

   <div className="mapModes">
     <button className={mapMode==='street'?'active':''} onClick={()=>changeMapMode('street')}>▦<span>Street</span></button>
     <button className={mapMode==='satellite'?'active':''} onClick={()=>changeMapMode('satellite')}>◉<span>Satellite</span></button>
     <button className={mapMode==='hybrid'?'active':''} onClick={()=>changeMapMode('hybrid')}>◈<span>Hybrid</span></button>
   </div>

   <button className="loc" onClick={()=>map.current?.flyTo(pos,17)}>◎</button>
   <div className="speed"><strong>{Math.round(speed)}</strong><span>MPH</span></div>

   {appMode==='drive'&&<>
     <section className="panel">
       <div className="tabs">
         <button className={activeTab==='drive'?'active':''} onClick={()=>setActiveTab('drive')}>ROUTE</button>
         <button className={activeTab==='convoy'?'active':''} onClick={()=>setActiveTab('convoy')}>CONVOY</button>
         <button className={activeTab==='garage'?'active':''} onClick={()=>setActiveTab('garage')}>GARAGE</button>
         <button className={activeTab==='history'?'active':''} onClick={()=>setActiveTab('history')}>DRIVES</button>
       </div>

       {activeTab==='drive'&&<>
         <div className="sectionTitle">ROUTE MODE</div>
         <div className="routeCards">
           <button className={routeMode==='fastest'?'active':''} onClick={()=>{setRouteMode('fastest');localStorage.setItem('gtiRouteMode','fastest')}}>
             <i>⚡</i><span><b>Fastest</b><small>Best ETA</small></span>
           </button>
           <button className={routeMode==='shortest'?'active':''} onClick={()=>{setRouteMode('shortest');localStorage.setItem('gtiRouteMode','shortest')}}>
             <i>⌖</i><span><b>Shortest</b><small>Least distance</small></span>
           </button>
           <button className={routeMode==='curvy'?'active beta':''} onClick={()=>{setRouteMode('curvy');localStorage.setItem('gtiRouteMode','curvy')}}>
             <i>〰</i><span><b>Curvy 🔥</b><small>Beta route picker</small></span><em>BETA</em>
           </button>
         </div>
         <div className="numbers">
           <span><b>{stats.dist.toFixed(2)}</b><small>MI</small></span>
           <span><b>{Math.round(stats.avg)}</b><small>AVG MPH</small></span>
           <span><b>{Math.round(stats.top)}</b><small>TOP MPH</small></span>
           <span><b>{stats.start?fmt(Date.now()-stats.start):'0m 0s'}</b><small>TIME</small></span>
         </div>
         <div className="buttons"><button className={tracking?'danger':''} onClick={drive}>{tracking?'END DRIVE':'START DRIVE'}</button></div>
       </>}

       {activeTab==='convoy'&&<div className="convoyPane">
         <div className="panelHero"><b>CONVOY</b><span className={convoyLive?'ok':''}>{convoyLive?`● LIVE · ${room}`:'○ OFFLINE'}</span></div>
         <div className="convoy"><input value={name} onChange={e=>setName(e.target.value)} placeholder="Driver name"/><input value={room} onChange={e=>setRoom(e.target.value.toUpperCase())} placeholder="Invite code"/><button onClick={()=>join()}>JOIN</button></div>
         <div className="convoyBtns"><button onClick={create}>CREATE CONVOY</button>{convoyLive&&<button onClick={leave}>LEAVE</button>}</div>
         <small className="hint">Everyone in the same code appears as their selected GTI with live MPH.</small>
       </div>}

       {activeTab==='garage'&&<div className="garage">
         <div className="panelHero"><b>GTI GARAGE</b><span>{carModel} · {carColor}</span></div>
         <div className="carChoices">
           {['MK5','MK6','MK7','MK7.5','MK8','GOLF R'].map(m=><button key={m} className={carModel===m?'active':''} onClick={()=>setCarModel(m)}>
             <img className={`garageCar color-${carColor}`} src={carAsset(m)}/><span>{m}</span>
           </button>)}
         </div>
         <div className="garageTitle">PAINT</div>
         <div className="colorChoices">{['silver','black','white','red','blue','yellow'].map(c=><button aria-label={c} key={c} className={`swatch ${c} ${carColor===c?'active':''}`} onClick={()=>setCarColor(c)}></button>)}</div>
         <small className="hint">Your model and paint are shared with convoy members.</small>
       </div>}

       {activeTab==='history'&&<div className="history">
         <div className="panelHero"><b>PREVIOUS DRIVES</b><span>{history.length} saved</span></div>
         {history.length===0?<div className="empty">No drives yet.<br/><small>End a drive and it will appear here.</small></div>:
         history.map(h=><div className="trip" key={h.id}><button className="tripMain" onClick={()=>openTrip(h)}>
           <span><strong>{new Date(h.started).toLocaleDateString()}</strong><small>{new Date(h.started).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'})}</small></span>
           <span><b>{h.distance.toFixed(1)} mi</b><small>{fmt(h.duration)}</small></span>
           <span><b>{Math.round(h.avg)} avg</b><small>{Math.round(h.top)} top</small></span>
         </button><button className="delete" onClick={()=>deleteTrip(h.id)}>×</button></div>)}
       </div>}
     </section>

     <section className="navCard">
       <div className="navArrow">↱</div>
       <div><small>CURRENT ROUTE</small><b>{dest?dest.split(',')[0]:'Choose a destination'}</b><span>{routeMode==='curvy'?'Curvy Beta':routeMode}</span></div>
       <div className="navSpeed"><b>{Math.round(speed)}</b><small>MPH</small></div>
     </section>
   </>}

   {appMode==='fish'&&<section className="fishPanel">
     <div className="fishHead"><div><b>🎣 NOVA FISH</b><small>PERSONAL FISHING INTELLIGENCE</small></div><button onClick={refreshFish}>{fishLoading?'…':'REFRESH'}</button></div>
     ...
     <div className="biteHero">
       <div className="biteScore"><strong>{fishData?.score??'--'}</strong><span>BITE SCORE</span></div>
       <div className="bestWindow"><small>BEST WINDOW</small><b>{fishData?.best?fishData.best.time.toLocaleTimeString([],{hour:'numeric',minute:'2-digit'}):'Waiting…'}</b><span>{fishData?.best?`${fishData.best.score}/100 predicted`:'Refresh current conditions'}</span></div>
     </div>
     ...
     <div className="fishSaved">
       <div className="panelHero"><b>SAVED SPOTS</b><span>{savedFishSpots.length}</span></div>
       {savedFishSpots.slice(0,4).map(s=><button key={s.id} onClick={()=>map.current?.flyTo([s.lat,s.lng],16)}><span>📍 {s.species}</span><small>{s.score?`${s.score}/100 · `:''}{s.date}</small></button>)}
       {!savedFishSpots.length&&<small className="hint">Save a spot and it will show here.</small>}
     </div>
     <div className="fishFoot"><span>DRIVE. EXPLORE. FISH.</span><span>{fishUpdated?`Updated ${new Date(fishUpdated).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'})}`:''}</span></div>
   </section>}
 </div>
}
